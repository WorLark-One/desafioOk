...
<table class="table table-sm table-striped">
	...
	<tbody>
		<?php $i=0;foreach($res->result() as $row):?>
		<tr>
			<?php if($rol == 1 && $row->estado == 1):?>
				<td><?php =$row->nombre?></td>
				<td><?php =$row->fecha?></td>
				<td><?php if(substr($row->nombre,-4) == ".txt"):?>
			...
			<?php if($rol == 2):?>
				...
				<td><a href="<?php =base_url().$row->ubicacion.$row->nombre?>" target="_blank" data-toggle="tooltip" title="Abrir"><button class="btn" style="padding: 0;"><i class="fas fa-external-link-alt"></i></button></a></td>
					<?php if($row->estado == 1):?>
